...
`takt` が所有するのは実行の移動であり、コンテキストの意味やアウトカム語彙ではありません。`iox` は `nil`、`ErrWouldBlock`、
`ErrMore`、失敗を分類します。`kont` はサスペンション/再開キャリアを所有します。`cove` は `SuspensionView`
によってサスペンションを明示的なコンテキストで包めます。`takt` は `SuspensionLike` を満たす値を進めますが、そのコンテキストは解釈しません。
